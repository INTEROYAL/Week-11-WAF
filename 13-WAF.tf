  resource "aws_wafv2_web_acl" "PentaCryptapp1_waf_acl" {
    name        = "PentaCryptapp1-web-acl"
    description = "Web ACL for PentaCryptapp1"
    scope       = "REGIONAL"
  ...
  rule {
   ...
   rule {
    ...
    statement {
      geo_match_statement {
        country_codes = ["DE"]  # Country code for Germany
      }
    }
    ...
    visibility_config {
      cloudwatch_metrics_enabled = false
      metric_name                = "PentaCryptapp1WebACL"
      sampled_requests_enabled   = false
    }

    tags = {
      Name = "PentaCryptapp1WAF"
 ...
 resource "aws_wafv2_web_acl_association" "PentaCryptapp1_waf_alb_association" {
  resource_arn = aws_lb.PentaCryptapp1_alb.arn
  web_acl_arn  = aws_wafv2_web_acl.PentaCryptapp1_waf_acl.arn
  }
